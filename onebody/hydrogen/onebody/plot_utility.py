...
def ufunc_3D_eles_mps (mps):
    # Define a function that only x and y are input parameters
    def _get_ele (bx, by, bz):
        # bx + by combine the binary strings to a single string
        return get_ele_mps (mps, bx+by+bz)

    # Return a ufunc
    return np.frompyfunc (_get_ele, 3, 1)
...
def get_3D_mesh_eles_mps (mps, bxs, bys, bzs):
    bX, bY, bZ = np.meshgrid (bxs, bys, bzs)
    get_3D_ele = ufunc_3D_eles_mps (mps)  # ufunc
    fs = get_3D_ele (bX, bY, bZ)
    fs = fs.astype(np.float64)
    return fs
