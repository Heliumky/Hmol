...
def one_over_r_2D (inds, fac, rescale, shift, cutoff):
    N = len(inds)
    inds_x = inds[:N//2]
    inds_y = inds[N//2:]
    ...
    return np.where(
        r < cutoff, 
        -fac/(0.5*cutoff),  # Approximation for small r
...
def run_tci (target_func, nsite, rescale, shift, cutoff, factor):
...
if __name__ == '__main__':
    ...
    if '--1D_one_over_r' in sys.argv:
        ...
        def target_func (inds):
            return one_over_r_2D (inds, factor, rescale, shift, cutoff)
        mps = run_tci (target_func, nsite, rescale, shift, cutoff, factor)
        plotF(mps, target_func, rescale, shift)
    ...
    def target_func (inds):
        return one_over_r (inds, factor, rescale, shift, cutoff)
