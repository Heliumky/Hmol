...
def npmpo_to_uniten (mpo,L,R):
    ...
    for i in range(len(mpo)):
        #print(f"mpo[{i}] dtype: {mpo[i].dtype}")
        h = ut.toUniTen(mpo[i])
...
def check_mpo_bonds (mpo, L, R):
    ...
    ut.check_same_bonds (L.bond('mid'), mpo[0].bond('l'))
    ut.check_same_bonds (R.bond('mid'), mpo[-1].bond('r'))
    for i in range(len(mpo)):
        assert set(mpo[i].labels()) == set(['l','ip','i','r'])
        ii = mpo[i].bond('i')
        iip = mpo[i].bond('ip')
        ut.check_same_bonds(ii,iip)

        if i != len(mpo)-1:
            b1 = mpo[i].bond("r")
            b2 = mpo[i+1].bond("l")
            ut.check_same_bonds(b1,b2)
...
def svd_bond_mpo (mpo, i, leftU, dim=sys.maxsize, cutoff=0.):
    ...
    A1, A2 = ut.decompose_tensor (AA, rowrank=3, dim=dim, cutoff=cutoff, leftU=leftU)
...
def svd_bond_mps (mps, i, leftU, dim=sys.maxsize, cutoff=0.):
    ...
    A1, A2 = ut.decompose_tensor (AA, rowrank=2, dim=dim, cutoff=cutoff, leftU=leftU)
