...
surfxy2 = ax[2].imshow(Z2, extent=(X.min(), X.max(), Y.min(), Y.max()), 
                       origin='lower', cmap='plasma')
ax[2].xaxis.set_major_locator(MaxNLocator(integer=True))
ax[2].set_xlabel(r'$x$', rotation=0, fontsize=25)
ax[2].set_ylabel(r'$y$', rotation=0, fontsize=25)
ax[2].set_aspect('equal', adjustable='box')
ax[2].set_title(f'potential')
ax[2].set_xlim(shift, -shift)
ax[2].set_ylim(shift, -shift)
ax[2].text(0.1, 0.9, "(c)", fontsize=25, transform=ax[2].transAxes)
ps.set_tick_inteval(ax[2].yaxis, major_itv=4, minor_itv=2)
ps.set_tick_inteval(ax[2].xaxis, major_itv=4, minor_itv=2)
ps.set(ax[2])
cbar2 = fig.colorbar(surfxy2, ax=ax[2])
