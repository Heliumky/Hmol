...
class LR_envir_tensors_mps:
    def __init__ (self, N, mps1, mps2):
        ...
        r2 = mps2[-1].bond("r")
        L0 = cytnx.UniTensor ([l1,l2], labels=['dn','up'],dtype = mps2[0].dtype())
        R0 = cytnx.UniTensor ([r1,r2], labels=['dn','up'],dtype = mps2[-1].dtype())
...
class eff_Hamilt (cytnx.LinOp):
    ...
    def matvec (self, v):
        #print("=== self.L ===")
        #print(self.L)
        #print("L dtype:", self.L.dtype())
        #print("L shape:", self.L.shape())
        psi = v.relabels(['l','i1','i2','r'],['ldn','i1','i2','rdn'])

        #print("=== psi ===")
        #print(psi)
        #print("psi dtype:", psi.dtype())
    ...
    for k in range(len(maxdims)):                                                            # For each sweep
        ...
        for lr in [0,1]:
            for p in ranges[lr]:                                                             # For each bond
                ...
                phi.set_rowrank_(2)
                s, u, vt, errs = cytnx.linalg.Svd_truncate(phi, keepdim=chi, err=cutoff, return_err=2)
                # s has labels _aux_L and _aux_R

                # Truncation error
                errs = ut.to_nparray(errs)
                terr += np.sum(errs)
